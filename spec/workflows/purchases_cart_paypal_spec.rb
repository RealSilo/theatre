...
describe PurchasesCartPaypal do
  describe 'successful credit card purchase', :vcr, :aggregate_failures do
    ...
    let(:ticket_1) { create(
      Ticket, status: 'waiting', price: Money.new(1500), performance: performance) }
    let(:ticket_2) { create(
      Ticket, status: 'waiting', price: Money.new(1500), performance: performance) }
    let(:ticket_3) { create(
    ...
    it 'updates the ticket status' do
      ...
      expect(ticket_3).to_not be_pending
      ...
      expect(workflow.payment.response_id).to eq(
        workflow.paypal_payment.paypal_payment.id
      )
    end
  end
end
